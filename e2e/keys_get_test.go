...
func testKeysGet(t *testing.T, client *t38c.Client) {
	require.NoError(t, client.Keys.Set("foo", "bar").Point(1, 2).Do())
	// Check object.
	{
	...
	// Check point.
	{
		resp, err := client.Keys.Get("foo", "bar").Point()
		require.NoError(t, err)
	...
	// Check bounds.
	{
		resp, err := client.Keys.Get("foo", "bar").Bounds()
		require.NoError(t, err)
	...
	// Check hash.
	{
		resp, err := client.Keys.Get("foo", "bar").Hash(5)
		require.NoError(t, err)
